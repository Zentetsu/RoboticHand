...
def addGround(rootNode):
    ...
    ground.addObject(
        "MeshSTLLoader",
        name="loader",
        filename=path + "Others/Ground.stl",
        rotation=[0, 0, 0],
        scale=1,
        translation=[0, 0, 0],
    )
...
def createScene(rootNode):
    ...
    rootNode.addObject(
        "LocalMinDistance", name="Proximity", alarmDistance=5, contactDistance=0.1
    )
    ...
    structure.addObject(
        "ParallelTetrahedronFEMForceField", poissonRatio=0.1, youngModulus=50
    )
    ...
    structure.addObject(
        "MeshSTLLoader", name="cube", filename=path + "Others/Cube.stl", scale=1
    )
    ...
    collision.addObject(
        "MechanicalObject", name="Collision_RM", translation=tran, rotation=rota
    )
